...
function resultRonRouteText(){
  const winner = lastAgariWinnerSeatIndex;
  const discarder = (typeof lastAgariDiscarderSeatIndex === "number")
    ? lastAgariDiscarderSeatIndex
    : null;
...
function arrangeResultTopRowLayout(){
  ...
  resultTopRowEl.style.gridTemplateColumns = "auto minmax(0, 1fr) auto";
  resultTopRowEl.style.alignItems = "center";
  resultTopRowEl.style.columnGap = "18px";
  resultTopRowEl.style.rowGap = "8px";
  ...
  if (resultInfoRowEl){
    resultInfoRowEl.style.display = "none";
  }

  if (resultDoraBlockEl){
    resultDoraBlockEl.style.display = "none";
  }

  if (resultUraDoraBlockEl){
    resultUraDoraBlockEl.style.display = "none";
  }
}
...
// - ronTile: ロン牌だけ独立表示
// ================================
function getResultWinnerHandParts(){
  const winner = lastAgariWinnerSeatIndex;
...
function getResultWinnerTiles(){
  const parts = getResultWinnerHandParts();
  const list = [];
...
function renderResultHandTiles(){
...
function getResultYakuInfo(){
  ...
  if (winner === 0){
    ...
    const ronTile = (winType === "ron") ? getSavedRonTileLike() : null;
    try{
  ...
  const totalCount = tiles14.length + meldList.reduce((sum, m)=>{
    ...
    if (m.type === "minkan" || m.type === "ankan" || m.type === "kakan") return sum + 4;
    return sum;
  }, 0);
...
function renderResultOverlay(){
  ...
  if (resultDoraBlockEl){
    resultDoraBlockEl.style.display = isRyukyoku ? "none" : "";
  }

  if (resultUraDoraBlockEl){
    resultUraDoraBlockEl.style.display = showUra ? "" : "none";
  }

  const topDoraGroupEl = document.getElementById("resultTopDoraGroup");
  if (topDoraGroupEl){
    topDoraGroupEl.style.display = isRyukyoku ? "none" : "flex";
  }

  const topUraGroupEl = document.getElementById("resultTopUraDoraGroup");
  if (topUraGroupEl){
    topUraGroupEl.style.display = showUra ? "flex" : "none";
  }
