...
function tileImgSrcByTile(tile){
  if (tile && tile.imgCode) return `img/${tile.imgCode}.png`;
  if (tile && tile.code) return `img/${tile.code}.png`;
  return `img/unknown.png`;
}
function haimenSrc(){
  return `img/haimen.png`;
}

// ===== タイル生成 =====
let nextId = 1;

function makeTile(code, imgCode = code){
  return {
    id: nextId++,
    code,
    imgCode,
...
function makeWall(){
  ...
  for (const code of TILE_TYPES){
    for (let i = 0; i < 4; i++){
      if (code === "5p" && i === 0){
        wall.push(makeTile("5p", "r5p"));
      } else if (code === "5s" && i === 0){
        wall.push(makeTile("5s", "r5s"));
      } else if (code === "4z" && i === 0){
        wall.push(makeTile("4z", "r4z"));
      } else {
        wall.push(makeTile(code));
      }
    }
  }
  return wall;
...
// ★ 王牌18枚の内部構成
// 0〜7   : 嶺上牌/北抜き補充に使う8枚
// 8〜12  : 表ドラ表示牌（初期1枚 + カンで最大4枚追加）
// 13〜17 : 裏ドラ表示牌（初期1枚 + カンで最大4枚追加）
// ================================
const DEAD_WALL_DRAW_COUNT = 8;
const DEAD_WALL_DORA_START = 8;
const DEAD_WALL_URA_START = 13;
const DEAD_WALL_INDICATOR_MAX = 5;
...
function canDeclareDoubleRiichiNow(seatIndex){
  ...
  if (Array.isArray(peis) && peis.length > 0) return false;
...
function getWinSituationFlags(winType, seatIndex){
  ...
  return {
    ...
    isHaitei: (winType === "tsumo") && !isRinshan && !isChankan && wallCount === 0,
    isHoutei: (winType === "ron") && !isChankan && wallCount === 0,
    isRinshan,
    isChankan,
...
function hasNorthInHand(){
  if (drawn && drawn.code === "4z") return true;
  return hand13.some(t => t.code === "4z");
}
...
function initWallsFromShuffled(shuffled108){
  // 末尾18枚を王牌にする
  deadWall = shuffled108.slice(-18);
  liveWall = shuffled108.slice(0, shuffled108.length - 18);
