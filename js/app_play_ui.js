...
(function(){
  ...
  function loadLaunchConfig(){
    ...
    if (launchMode === "verify") playerMode = "auto";
  ...
  function injectModeStyles(){
    ...
    style.textContent = `
      ...
      .appModeBadge.isBattleMode{
        border-color: rgba(243,215,142,0.24);
        background: rgba(50, 36, 8, 0.78);
        color: #ffe7a7;
      }
      .appModeBadge.isVerifyMode{
        border-color: rgba(129,225,255,0.24);
  ...
  function syncModeBadgePosition(){
    ...
    const anchor = document.getElementById("activeAccountBadge") || document.getElementById("appTopBackBtn");
    if (!anchor || typeof anchor.getBoundingClientRect !== "function") return;

    const rect = anchor.getBoundingClientRect();
    if (!rect || !(rect.width > 0 || rect.height > 0)) return;

    const gapY = (window.matchMedia && window.matchMedia("(orientation: landscape) and (max-height: 700px)").matches) ? 8 : 10;

    badge.style.left = `${Math.round(rect.left)}px`;
    badge.style.right = "auto";
    badge.style.top = `${Math.round(rect.bottom + gapY)}px`;
  }
  ...
  function applyTopButtonMode(debugBtn, settingsBtn, logBtn){
    ...
    const hiddenTargets = [debugBtn, settingsBtn, logBtn];
    hiddenTargets.forEach((btn)=>{
      if (!btn) return;
      const shouldHide = !!currentLaunchConfig.isBattleMode;
...
function installTopButtons(){
    ...
    const debugBtn = document.getElementById("debugOpenBtn");
    ...
    applyTopButtonMode(debugBtn, settingsBtn, logBtn);
  }
  ...
  function scheduleVerifyModeNextHanchan(){
    ...
    }, VERIFY_MODE_NEXT_HANCHAN_DELAY_MS);
  }
  ...
  function installEscapeClose(){
    document.addEventListener("keydown", (ev)=>{
      ...
      ["visitorRuleOverlay", "visitorStatsOverlay", "visitorReplayOverlay"].forEach((id)=>{
