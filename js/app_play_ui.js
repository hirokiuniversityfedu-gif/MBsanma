...
(function(){
  ...
  function normalizeStatsTabKey(value){
    const key = String(value || "overview");
    return ["overview", "graphs", "detail", "cpu", "logs"].includes(key) ? key : "overview";
  }

  function setActiveStatsTabKey(value){
    activeStatsTabKey = normalizeStatsTabKey(value);
    return activeStatsTabKey;
  }
  ...
  function getNormalizedLogContext(logs){
    ...
    const normalizedLogs = normalizer && typeof normalizer.normalizeStoredLogs === "function"
      ? normalizer.normalizeStoredLogs(logs)
      : [];
    const summary = metricsApi && typeof metricsApi.summarizeLogs === "function"
      ? metricsApi.summarizeLogs(normalizedLogs)
      : null;
    return { normalizedLogs, summary, normalizer };
  }

  function buildLogMetricsPanel(summary){
    const safeSummary = (summary && typeof summary === "object") ? summary : {};
    return buildStatsMetricPanel("ログ集計", [
      { label: "局数", value: `${Number(safeSummary.kyokuCount) || 0}`, hint: "ログ化された局数" },
      { label: "打牌", value: `${Number(safeSummary.playerDiscardCount) || 0}`, hint: "あなたの打牌" },
      { label: "リーチ率", value: formatPercentValue(safeSummary.playerRiichiRate), hint: "局数基準" },
      { label: "アガリ率", value: formatPercentValue(safeSummary.playerAgariRate), hint: "局数基準" },
      { label: "放銃率", value: formatPercentValue(safeSummary.playerHojuRate), hint: "局数基準" },
      { label: "副露率", value: formatPercentValue(safeSummary.playerFuroRate), hint: "局数基準" },
      { label: "北抜き", value: `${Number(safeSummary.playerPeiCount) || 0}`, hint: "ログ件数" },
      { label: "見送り率", value: formatPercentValue(safeSummary.playerPassRate), hint: "鳴き選択基準" },
      { label: "CPU一致率", value: formatPercentValue(safeSummary.cpuDiscardShadowAgreeRate), hint: "shadow比較" },
      { label: "CPU打牌判断", value: `${Number(safeSummary.cpuDiscardCount) || 0}`, hint: "束ね後件数" }
    ]);
  }


  function getTopCountEntries(map, limit = 6){
    const src = (map && typeof map === "object") ? map : {};
    return Object.keys(src)
      .map((key)=> ({ key, count: Number(src[key]) || 0 }))
      .filter((item)=> item.count > 0)
      .sort((a, b)=> b.count - a.count || a.key.localeCompare(b.key, "ja"))
      .slice(0, Math.max(0, limit | 0));
  }

  function formatExecutionSourceLabel(key){
    const map = {
      external: "external",
  ...
  function buildCounterListPanel(title, rows, emptyText){
    const panel = document.createElement("div");
    ...
    if (!Array.isArray(rows) || rows.length <= 0){
      ...
      empty.textContent = emptyText || "まだ件数がありません。";
      panel.appendChild(empty);
      return panel;
    }

    const grid = document.createElement("div");
    grid.className = "visitorRankBars";

    rows.forEach((row)=>{
      const wrap = document.createElement("div");
      wrap.className = "visitorRankBarRow";
      ...
      fill.style.width = `${Math.max(0, Math.min(100, Number(row && row.rate) || 0))}%`;
      fill.style.background = row && row.color ? row.color : "#eac75f";
      track.appendChild(fill);

      const value = document.createElement("div");
      value.className = "visitorRankBarValue";
      value.textContent = `${Number(row && row.count) || 0}`;
  ...
  function buildCpuAnalysisPanel(summary){
    const safeSummary = (summary && typeof summary === "object") ? summary : {};
    const panel = document.createElement("section");
    panel.className = "visitorStatsTabPanel";
    panel.dataset.statsPanelKey = "cpu";

    const intro = document.createElement("div");
    intro.className = "visitorStatsEmpty";
    intro.innerHTML = "ここは <strong>CPU判断ログの確認口</strong> です。<br>いまは shadow一致率・実行ソース比率・reasonTag上位を見られるようにしています。";
    panel.appendChild(intro);

    panel.appendChild(buildStatsMetricPanel("CPU分析サマリー", [
      { label: "CPU打牌判断", value: `${Number(safeSummary.cpuDiscardCount) || 0}`, hint: "束ね後件数" },
      { label: "CPU副露判断", value: `${Number(safeSummary.cpuOpenCount) || 0}`, hint: "束ね後件数" },
      { label: "shadow比較", value: `${Number(safeSummary.cpuDiscardShadowComparedCount) || 0}`, hint: "一致率対象" },
      { label: "shadow一致率", value: formatPercentValue(safeSummary.cpuDiscardShadowAgreeRate), hint: "打牌判断" },
      { label: "player見送り率", value: formatPercentValue(safeSummary.playerPassRate), hint: "鳴き選択基準" },
      { label: "player鳴き選択", value: `${Number(safeSummary.playerCallPromptCount) || 0}`, hint: "prompt件数" }
    ]));

    const executionRows = getTopCountEntries(safeSummary.cpuDiscardExecutionSourceCounts, 6);
    const executionTotal = executionRows.reduce((sum, row)=> sum + row.count, 0);
    panel.appendChild(buildCounterListPanel(
      "打牌実行ソース",
      executionRows.map((row)=> ({
        label: formatExecutionSourceLabel(row.key),
        count: row.count,
        rate: executionTotal > 0 ? (row.count / executionTotal) * 100 : 0,
        color: row.key === "external" ? "#73d9ff" : (row.key === "legacy" ? "#e46d3f" : "#67e28d")
      })),
      "まだ executionSource の件数がありません。"
    ));

    const discardTagRows = getTopCountEntries(safeSummary.cpuDiscardReasonTagCounts, 8);
    const discardTagTotal = discardTagRows.reduce((sum, row)=> sum + row.count, 0);
    panel.appendChild(buildCounterListPanel(
      "打牌 reasonTag 上位",
      discardTagRows.map((row)=> ({
        label: row.key,
        count: row.count,
        rate: discardTagTotal > 0 ? (row.count / discardTagTotal) * 100 : 0,
        color: "#f7b343"
      })),
      "まだ打牌 reasonTag がありません。"
    ));

    const openTagRows = getTopCountEntries(safeSummary.cpuOpenReasonTagCounts, 8);
    const openTagTotal = openTagRows.reduce((sum, row)=> sum + row.count, 0);
    panel.appendChild(buildCounterListPanel(
      "副露 reasonTag 上位",
      openTagRows.map((row)=> ({
        label: row.key,
        count: row.count,
        rate: openTagTotal > 0 ? (row.count / openTagTotal) * 100 : 0,
        color: "#c27cff"
      })),
      "まだ副露 reasonTag がありません。"
    ));

    return panel;
  }

  function buildLogOnlyOverviewPanel(summary){
    const wrap = document.createElement("section");
    wrap.className = "visitorStatsTabPanel";
    wrap.dataset.statsPanelKey = "overview";

    const empty = document.createElement("div");
    empty.className = "visitorStatsEmpty";
    empty.innerHTML = "半荘成績の保存はまだありません。<br>先にログだけ取れているので、暫定のログ集計を表示しています。";
    wrap.appendChild(empty);
    wrap.appendChild(buildLogMetricsPanel(summary));
    return wrap;
  }
  ...
  function buildLogsPanel(logs){
    ...
    const logContext = getNormalizedLogContext(logs);
    const normalizedLogs = Array.isArray(logContext.normalizedLogs) ? logContext.normalizedLogs : [];
    const normalizer = logContext.normalizer;
  ...
  function renderStatsOverlay(){
    ...
    const logContext = getNormalizedLogContext(storedLogs);
    const logSummary = logContext.summary;
    ...
    const availableTabKeys = history.length
      ? ["overview", "graphs", "detail", "cpu", "logs"]
      : (storedLogs.length ? ["overview", "cpu", "logs"] : ["logs"]);
    const activeKey = availableTabKeys.includes(normalizeStatsTabKey(activeStatsTabKey)) ? normalizeStatsTabKey(activeStatsTabKey) : availableTabKeys[0];
    ...
    if (history.length){
      ...
      if (logSummary) overviewPanel.appendChild(buildLogMetricsPanel(logSummary));
      ...
      if (logSummary) detailPanel.appendChild(buildLogMetricsPanel(logSummary));

      const cpuPanel = buildCpuAnalysisPanel(logSummary);
      panelEls.push(overviewPanel, graphsPanel, detailPanel, cpuPanel);
      panels.appendChild(overviewPanel);
      panels.appendChild(graphsPanel);
      panels.appendChild(detailPanel);
      panels.appendChild(cpuPanel);
    } else if (storedLogs.length){
      const overviewPanel = buildLogOnlyOverviewPanel(logSummary);
      const cpuPanel = buildCpuAnalysisPanel(logSummary);
      panelEls.push(overviewPanel, cpuPanel);
      panels.appendChild(overviewPanel);
      panels.appendChild(cpuPanel);
    }
  ...
  function installTopButtons(){
    ...
    let ruleBtn = document.getElementById("appRuleBtn");
    ...
    let statsBtn = document.getElementById("appStatsBtn");
