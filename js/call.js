...
function __callBuildOpenMeld(type, code, from, removedTiles, calledTile){
...
function buildPlayerCallAiSnapshot(tile, from, legalActions){
  ...
  return {
    ...
    discarderSeatIndex,
    legalActions: {
      pon: !!(legalActions && legalActions.pon),
...
function renderCallButtons(){
  ...
  if (typeof setActionButtonState === "function"){
    ...
    setActionButtonState(passBtn, inCall && !aiControlled, inCall && !aiControlled);
...
function choosePon(doIt){
  ...
  // 副露として保持（from付き）
  melds.push(__callBuildOpenMeld("pon", code, from, removedTiles, calledTile));
  openPonEffect(0);
...
function chooseMinkan(doIt){
  ...
  // 副露として保持（from付き）
  melds.push(__callBuildOpenMeld("minkan", code, from, removedTiles, calledTile));
