...
let verifyFastModeEnabled = false;
...
function isVerifyFastModeEnabled(){
  return isVerifyLaunchModeNow() && !!verifyFastModeEnabled;
}

function setVerifyFastModeEnabled(value){
  verifyFastModeEnabled = !!value;
  try{
  ...
  return verifyFastModeEnabled;
}

function getCurrentGameSpeedProfile(){
  return isVerifyFastModeEnabled() ? GAME_SPEED_PROFILE_VERIFY_FAST : GAME_SPEED_PROFILE_NORMAL;
}
...
function startDebugKyokuByScenario(opts){
  try{
    ...
    if (!Array.isArray(remainingWall) || remainingWall.length <= 0) return false;
...
function buildDisplaySettingsPaneHtml(){
  ...
  const verifyFast = !!verifyFastModeEnabled;
    ...
    <div class="settingsSection">
      ...
      ${isVerifyMode ? `
        <div class="settingsModeSummary" style="margin-top:12px;">
          <label class="settingsCheck" style="font-weight:800;">
            <input type="checkbox" id="settingsVerifyFastMode"${verifyFast ? " checked" : ""}>
            <span>高速モード</span>
          </label>
          <div class="settingsModeSummaryItem">
...
function renderGameSettingsPanel(){
  ...
  const verifyFastModeInput = document.getElementById("settingsVerifyFastMode");
  if (verifyFastModeInput){
    verifyFastModeInput.addEventListener("change", ()=>{
      setVerifyFastModeEnabled(!!verifyFastModeInput.checked);
      saveGameSettingsToStorage();
...
function collectGameSettingsForStorage(){
  return {
    ...
    verifyFastMode: !!verifyFastModeEnabled,
    player: {
...
function applyGameSettingsFromObject(raw){
  ...
  try{
    if (typeof src.verifyFastMode !== "undefined"){
      setVerifyFastModeEnabled(!!src.verifyFastMode);
    } else {
      setVerifyFastModeEnabled(false);
    }
  }catch(e){}
