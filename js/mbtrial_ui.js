(function(){
  ...
  function markCouponUsed(){
    ...
    const coupon = loadCouponRecord();
    if (!coupon) return;
    coupon.status = "used";
    saveCouponRecord(coupon);
  }
  ...
  function issueCouponIfNeeded(endInfo, settlement){
    const existing = loadCouponRecord();
  ...
  function ensureGuideOverlay(){
    ...
    root.innerHTML = `
      <div class="mbTrialGuideCard">
        ...
        <div class="mbTrialGuideLine">店外版は <strong>1半荘チャレンジ</strong> 専用です。半荘終了後、着順に応じて来店特典クーポンを発行します。</div>
      </div>
      <div class="mbTrialGuideCard">
      ...
      <div class="mbTrialGuideCard">
        <div class="mbTrialGuideCardTitle">スクショを撮り忘れても大丈夫</div>
  ...
  function renderCouponOverlay(options){
    ...
    if (fromEnd){
      ...
      topBtn.addEventListener("click", ()=>{
        clearMbTrialSession();
        location.href = "index_mbtrial.html";
      });
  ...
  function openCouponOverlay(options){
    renderCouponOverlay(options);
  }
  ...
  function installTopPageButtons(){
    ...
    if (startBtn){
      startBtn.addEventListener("click", ()=>{
        startMbTrialSession();
        location.href = "play_mbtrial.html";
      });
    }
    ...
    overlay.addEventListener("click", ()=>{
      ...
      window.setTimeout(()=>{
        openCouponOverlay({
          ...
          newlyIssued: context.newlyIssued,
          fromEnd: true
        });
      }, 0);
    }, true);
  ...
  function installSettlementHooks(){
    if (typeof showHanchanEndOverlay === "function" && !showHanchanEndOverlay.__mbTrialWrapped){
      ...
      const wrappedShow = function(endInfo, settlement){
        ...
        try{
          const issueResult = issueCouponIfNeeded(endInfo, settlement);
          pendingHanchanCouponContext = {
  ...
  function exposeDebugHelpers(){
    try{
      window.markMbTrialCouponUsed = markCouponUsed;
