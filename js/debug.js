...
(function(){
  ...
  const DEBUG_PRESET_STORAGE_KEY = 'mbsanma_debug_scenario_v4';
  ...
  function getTargetMax(target){
    return target === 'dora' ? DEBUG_DORA_MAX : DEBUG_HAND_MAX;
  }
  ...
  function loadLastPreset(){
    try{
      ...
      if (!obj || typeof obj !== 'object' || obj.v !== 4) return null;
      return {
  ...
  function renderTargetTabs(){
    ...
    for (const target of ['me', 'right', 'left', 'dora']){
  ...
  function renderDebugSlots(){
    ...
    for (let i = 0; i < max; i++){
      ...
      if (i < list.length){
        ...
        label.textContent = (state.activeTarget === 'dora') ? 'ドラ' : `${i + 1}`;
  ...
  function buildDebugOverlay(){
    ...
    sub.textContent = '未指定の手牌・ドラはランダム補完。巡目ぶんの河もランダム生成して、親のツモ番から開始。';
