...
function getCurrentRiichiDepositorSeats(){
  ...
  for (let seat = 0; seat < 3; seat++){
    try{
      if (typeof isSeatRiichiNow === "function" && isSeatRiichiNow(seat)){
        seats.push(seat);
      }
    }catch(e){}
  }
...
function buildCurrentRoundSettlement(){
  if (pendingRoundSettlement) return pendingRoundSettlement;

  let settlement = null;

  if (lastAgariType === "tsumo" || lastAgariType === "ron"){
  ...
  if (settlement){
    pendingRoundSettlement = settlement;
  }
