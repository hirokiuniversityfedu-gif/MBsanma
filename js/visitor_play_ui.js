...
(function(){
  ...
  const ACTIVE_SESSION_STORAGE_KEY = "mbsanma_visitor_active_session_v1";
  const ACCOUNT_REGISTRY_STORAGE_KEY = "mbsanma_visitor_account_registry_v1";
  ...
  const GUEST_HISTORY_STORAGE_KEY = "mbsanma_visitor_guest_history_v1";
  const GUEST_TRACKER_STORAGE_KEY = "mbsanma_visitor_guest_tracker_v1";
  const ACCOUNT_KEY_PREFIX = "mbsanma_visitor_account_";
  ...
  const currentSession = loadVisitorSession();
  ...
  function normalizeVisitorSession(src){
    const mode = src && src.mode === "account" ? "account" : "guest";
  ...
  function loadVisitorSession(){
    try{
      ...
      if (raw){
        return normalizeVisitorSession(JSON.parse(raw));
      }
    }catch(e){}
    return { mode: "guest", accountId: "" };
  }

  function getScopedStorageEngine(){
    return currentSession.mode === "account" ? localStorage : sessionStorage;
  }
  ...
  function getScopedHistoryStorageKey(){
    ...
    return GUEST_HISTORY_STORAGE_KEY;
  }
  ...
  function getScopedTrackerStorageKey(){
    ...
    return GUEST_TRACKER_STORAGE_KEY;
  }
  ...
  async function pushSnapshotToRemote(){
    ...
    try{
      ...
      if (result && result.error){
        console.warn("[visitor_play_ui] remote sync failed:", result.error.message || result.error);
      }
    }catch(error){
      console.warn("[visitor_play_ui] remote sync failed:", error);
    }finally{
