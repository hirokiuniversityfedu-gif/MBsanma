...
(function(global){
  ...
  function readActiveSession(){
    ...
    if (loginAccountId){
      return { mode: "account", accountId: loginAccountId };
    }
    ...
    const kyokus = Array.isArray(log.kyokus) ? log.kyokus.map((kyoku)=> {
      ...
      const events = Array.isArray(src.events)
        ? src.events.filter((event)=> !isHeavyEventTypeForStorage(String(event && event.type || ""))).map((event)=> ({
            ...
            payload: clonePlainData(event && event.payload)
          }))
        : [];
  ...
  function persistCompletedLog(log){
    ...
    try{
      ...
      const next = [optimizedLog, ...list.filter((item)=> item.matchId !== optimizedLog.matchId)].slice(0, MAX_STORED_MATCHES);
      localStorage.setItem(storageKey, JSON.stringify(next));
      return true;
    }catch(e){
      try{
        console.warn("[match_log] persistCompletedLog failed", {
          storageKey,
  ...
  function finishMatch(endInfo, settlement){
    ...
    log.updatedAt = safeNowIso();
    log.persisted = !!persistCompletedLog(log);
    return log;
  }
  ...
  function getStoredLogs(){
    ...
    const next = [currentLog, ...list.filter((item)=> item && item.matchId !== currentLog.matchId)];
    return next;
  }
