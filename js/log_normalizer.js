...
(function(global){
  ...
  function buildDefaultEntry(event){
    ...
    let detail = "";
    let extra = "";
    let seatIndex = Number.isInteger(payload.seatIndex) ? payload.seatIndex : null;
    let winnerSeatIndex = Number.isInteger(payload.winnerSeatIndex) ? payload.winnerSeatIndex : null;
    let discarderSeatIndex = Number.isInteger(payload.discarderSeatIndex) ? payload.discarderSeatIndex : null;
    let reasonTag = payload.reasonTag || "";
    let executionSource = payload.executionSource || "";
    let shadowAgree = payload.shadowAgree === true;
    ...
    if (type === "draw"){
      ...
      if (payload.isRiichiDeclare) extra = "リーチ宣言牌";
    }else if (type === "pei"){
      title = `${getSeatLabel(payload.seatIndex)} 北抜き`;
    }else if (type === "pon" || type === "call_pon"){
      title = `${getSeatLabel(payload.seatIndex)} ポン ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
      if (Number.isInteger(payload.fromSeatIndex)) detail = `from=${getSeatLabel(payload.fromSeatIndex)}`;
    }else if (type === "minkan" || type === "call_minkan"){
      ...
      title = `${getSeatLabel(payload.seatIndex)} リーチ`;
      const bits = [];
      if (payload.declareTile && payload.declareTile.code) bits.push(`tile=${payload.declareTile.code}`);
      if (payload.source) bits.push(`source=${payload.source}`);
      if (payload.turnPhase) bits.push(`phase=${payload.turnPhase}`);
      detail = bits.join(" / ");
    }else if (type === "call_prompt"){
      title = `${getSeatLabel(payload.seatIndex)} 鳴き選択`;
      const acts = [];
      if (payload.canRon) acts.push("ロン");
      if (payload.canMinkan) acts.push("明槓");
      if (payload.canPon) acts.push("ポン");
      const bits = [];
      if (payload.code) bits.push(`tile=${payload.code}`);
      if (Number.isInteger(payload.fromSeatIndex)) bits.push(`from=${getSeatLabel(payload.fromSeatIndex)}`);
      if (acts.length) bits.push(`候補:${acts.join("/")}`);
      detail = bits.join(" / ");
      if (payload.aiControlled) extra = "AI選択";
    }else if (type === "call_pass"){
      title = `${getSeatLabel(payload.seatIndex)} 見送り`;
      const bits = [];
      if (payload.code) bits.push(`tile=${payload.code}`);
      if (Number.isInteger(payload.fromSeatIndex)) bits.push(`from=${getSeatLabel(payload.fromSeatIndex)}`);
      if (payload.reason) bits.push(`reason=${payload.reason}`);
      detail = bits.join(" / ");
      ...
      title = `${winners} ロン`;
      if (Array.isArray(payload.winnerSeatIndexes) && payload.winnerSeatIndexes.length){
        winnerSeatIndex = payload.winnerSeatIndexes[0];
      }
      ...
      if (Number.isInteger(settlement.winnerSeatIndex)) winnerSeatIndex = settlement.winnerSeatIndex;
      if (Number.isInteger(settlement.discarderSeatIndex)) discarderSeatIndex = settlement.discarderSeatIndex;
  ...
  function buildCpuDiscardEntry(bundle){
    ...
    return {
      ...
      reasonTag: lastDecisionPayload.reasonTag || "",
      executionSource: lastDecisionPayload.executionSource || "",
      shadowAgree: lastDecisionPayload.shadowAgree === true
    };
  }
