...
(function(global){
  ...
  function buildDefaultEntry(event){
    ...
    if (type === "draw"){
      ...
      const bits = [];
      if (Number(payload.junme) > 0) bits.push(`${Number(payload.junme)}巡目`);
      if (payload.riichiKind === "double") bits.push("ダブリー");
      if (payload.declareTile && payload.declareTile.code) bits.push(`宣言牌=${payload.declareTile.code}`);
      detail = bits.join(" / ");
      if (payload.tenpai && Array.isArray(payload.tenpai.waitCodes) && payload.tenpai.waitCodes.length){
        extra = `待ち=${payload.tenpai.waitCodes.join(",")} / ${Array.isArray(payload.tenpai.waitTypeKeys) ? payload.tenpai.waitTypeKeys.join(",") : ""} / 残り${Number(payload.tenpai.waitTileCount) || 0}枚`;
      }
    }else if (type === "call_prompt"){
      title = `${getSeatLabel(payload.seatIndex)} 鳴き選択 ${payload.code || "?"}`;
      detail = `候補:${payload.canRon ? "ロン " : ""}${payload.canMinkan ? "明槓 " : ""}${payload.canPon ? "ポン " : ""}`.trim();
      extra = `from=${getSeatLabel(payload.fromSeatIndex)} / ${Number(payload.junme) || 0}巡目`;
    }else if (type === "call_pass"){
      title = `${getSeatLabel(payload.seatIndex)} 鳴き見送り ${payload.code || "?"}`;
      detail = `from=${getSeatLabel(payload.fromSeatIndex)} / ${Number(payload.junme) || 0}巡目`;
      if (payload.reason) extra = `reason=${payload.reason}`;
    }else if (type === "call_pon" || type === "pon"){
      title = `${getSeatLabel(payload.seatIndex)} ポン ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
    }else if (type === "call_minkan" || type === "minkan"){
      title = `${getSeatLabel(payload.seatIndex)} 明槓 ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
      if (Number(payload.junme) > 0) detail = `${Number(payload.junme)}巡目`;
    }else if (type === "ankan"){
      ...
      if (scoreInfo){
        const bits = [];
        if (Number.isFinite(scoreInfo.han)) bits.push(`${scoreInfo.han}翻`);
        if (Number.isFinite(scoreInfo.fu)) bits.push(`${scoreInfo.fu}符`);
        if (Number.isFinite(scoreInfo.ronPoint)) bits.push(`ron=${scoreInfo.ronPoint}`);
        if (Number.isFinite(scoreInfo.tsumoPointKo) || Number.isFinite(scoreInfo.tsumoPointOya)) bits.push(`tsumo=${Number(scoreInfo.tsumoPointKo) || 0}/${Number(scoreInfo.tsumoPointOya) || 0}`);
        extra = bits.join(" / ");
      }
  ...
  function buildCpuDiscardEntry(bundle){
    ...
    return {
      ...
      shadowAgree: !!lastDecisionPayload.shadowAgree
    };
  }
